...
function search(inputValue) {
  ...
  //   },
    success: function (res) {
      ...
      app.data.length = res.data.length;
      app.data.imageAllList=[];
      app.data.searchnameImage = [];
      app.data.searchstyleImage= [];
      for (let i = 0; i < res.data.length; i++) {
        // 得到的是数组长度
        var a = app.data.imageAllList.push(res.data[i].url)
        var b = app.data.searchnameImage.push(res.data[i].name)
        var c = app.data.searchstyleImage.push(res.data[i].style)
        //  必须console
        // console.log(app.data.imageAllList)
        // console.log(app.data.searchnameImage)
        // console.log(app.data.searchstyleImage)
      }
      // 传递给新的数组
      // var temData = __that.data.wxSearchData;
      temData.imageList = app.data.imageAllList;
      // console.log(temData.imageList)
      __searchFunction(temData.imageList);
